...
class Generator extends \yiiframe\gii\Generator
{
    ...
    public function beforeValidate()
    {
        ...
        // 先 Base64 解码，再 JSON 解码
        if ($this->listFields) {
            $json = base64_decode($this->listFields);
            $this->listFields = json_decode($json, true) ?: [];
        }
        if ($this->formFields) {
            $json = base64_decode($this->formFields);
            $this->formFields = json_decode($json, true) ?: [];
        }
        if ($this->inputType) {
            $json = base64_decode($this->inputType);
            $this->inputType = json_decode($json, true) ?: [];
        }
        /* 如果是字符串（CLI），就解码 */
