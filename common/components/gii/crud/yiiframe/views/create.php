...
$safeAttributes = array_diff($safeAttributes, ['merchant_id', 'member_id', 'created_at', 'updated_at']);
echo "<?php\n";
?>
...
$this->title = <?= $generator->generateString(Inflector::camel2words(Yii::t('app','创建'))) ?>;
$this->params['breadcrumbs'][] = ['label' => <?= $generator->generateString($generator->getTableComment()) ?>, 'url' => ['index']];
...
if (!empty($generator->formFields)) {
    foreach ($generator->formFields as $attribute) {
        ...
        elseif($attribute=='member_id')
            echo "            [
                'attribute' => 'member_id',
                'value' => function (\$model) {
                    return \yii\helpers\ArrayHelper::getValue(Yii::\$app->services->devPattern->member()::dropDown(),\$model->member_id,'未知');
                },
